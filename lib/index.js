...
function Multi(engine, opts) {
		...
		*/
		...
		tabs.on('message', function(sender, data) {
			...
			if (data.uri == uri && data.type == "isMaster") {
				...
				console.log("Delegating to tab "+sender)

				socket.master = new parent.Delegator(sender, uri)
				self.callAll()
	...
	this.Delegator = function(sender, uri) {
		...
		parent.tabs.on('message', function(sender, data) {
			if (sender == self.tab && data.uri == self.uri) {
				switch (data.type) {
				case "event":
					// TODO: this?
