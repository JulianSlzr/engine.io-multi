...
function Multi(engine, opts) {
	...
	this.Socket.prototype.send = function() {
		this.master.send.apply(this.master, arguments);
	};
	...
	this.Socket.prototype.becomeMaster = function(uri) {
		...
		console.log("Becoming master for "+uri+"...")
		...
		self.parent.tabs.on('message', function(sender, data) {
			if (self.uri == data.uri) {
				switch (data.type) {
				case "call":
					console.log("communicating for "+data.args[0]);
					...
					result = self.master[data.fn].apply(self.master, data.args);
	...
	this.Delegator.prototype.send = function() {
		this.socket.parent.tabs.send(this.tab, {
			uri: this.uri,
			type: "call",
			fn: "send",
